...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS remainder_left FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING remainder_left >= 0"
end
